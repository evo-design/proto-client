...
`utils/http.py` implements transport-level retries. Retryable: `{429, 500, 502, 503, 504}` + network/timeout errors + `httpx.RemoteProtocolError` (server-side keep-alive disconnect on long polls). `LocalProtocolError` (client-side bug), `DecodingError`, and `InvalidURL` stay non-retriable. Client errors (400, 401, 403, 404, 409, 422) are never retried. Tuning lives in `RetryConfig` (`utils/http.py`): conservative defaults, exponential backoff with jitter, and an honored server `Retry-After` (429/503) capped at `retry_after_max`. Only idempotent methods retry unconditionally; a POST retries only with an `Idempotency-Key` (auto-generated by `tools.run`/`run_batch`; `runs` send none until the runs API adds idempotency support).
